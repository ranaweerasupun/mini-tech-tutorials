...
In the previous document you learned that the kernel's bus subsystem acts as a matchmaker between drivers and devices. But that raises an obvious question: how does the kernel know what devices exist in the first place? For a USB keyboard, the USB protocol includes device enumeration — the device announces itself. For a chip sitting on a two-wire I2C bus, there's no such mechanism. The bus has no discovery protocol. The sensor doesn't announce its presence. The kernel needs to be *told* the hardware is there.

That's the problem device trees solve.

---

## What a Device Tree Is

A **device tree** is a data structure that describes the hardware layout of a computer to the operating system. It's written in a human-readable language called DTS (Device Tree Source), compiled into a binary DTB (Device Tree Blob), and loaded into memory by the bootloader before the kernel starts. When the kernel boots, one of its very first acts is to read the device tree and learn about the hardware it's running on.

Think of the device tree as a wiring diagram in text form. It describes which buses exist (I2C bus 1, SPI bus 0), what's connected to each bus (a temperature sensor at address 0x48), and what software should handle each device (`compatible = "ti,tmp102"`). The hardware description (the device tree) is deliberately kept separate from the software that drives it (the kernel module). This separation is what allows a single kernel binary to run on many different hardware configurations — the device tree changes per board, but the kernel itself doesn't.

---

## The Compatible String: The Critical Link

Every node in a device tree that represents a piece of hardware has a `compatible` property — a string that identifies what kind of device it is. The convention is `"vendor,model"`: `"ti,tmp102"` for Texas Instruments' TMP102, `"bosch,bmp280"` for Bosch's BMP280 pressure sensor, and so on.
...
When the kernel reads the device tree and finds a node with `compatible = "ti,tmp102"`, it searches all registered drivers for one whose `of_match_table` contains that string. When it finds your driver, it calls your `probe` function. The compatible string is the handshake that makes this all work — getting it wrong (a typo, wrong capitalisation, hyphen vs underscore) is one of the most common reasons a driver's `probe` is never called. It's also one of the harder bugs to spot, so it's worth double-checking.

---

## Overlays: Patching the Device Tree at Boot

The base device tree for a Raspberry Pi describes the Pi's own hardware — the CPU, the memory controller, the built-in peripherals. It knows nothing about accessories you've connected to the GPIO header, because those are specific to your project, not to the Pi itself.

A **device tree overlay** is a patch applied to the base device tree at boot time, adding or modifying nodes. Instead of editing the Pi's base device tree (which would need to be redone after every kernel update), you write a small overlay that describes just your sensor and tell the bootloader to apply it. The bootloader merges it with the base tree before handing control to the kernel.

---

## Anatomy of an Overlay File

Here is the overlay for the TMP102 sensor, which you'll also find in the examples directory. With the context from this document, every line should make sense:
...
Notice that the sensor node (`tmp102@48`) is nested inside the I2C bus node (`i2c1`). This parent-child relationship tells the kernel that this sensor lives on this particular I2C bus — it's a child device of that bus. The kernel's I2C bus driver will see this child node, match its compatible string to your driver, and call your `probe` function with an `i2c_client` struct populated from the node's properties — including the I2C address from the `reg` field.

---

## Compiling and Installing the Overlay

The DTS source is human-readable, but the kernel needs it in compiled binary (DTBO) format. The device tree compiler `dtc` handles this:
...
Then tell the bootloader to apply it at boot:
...
One common question at this point: why does `/sys/bus/i2c/devices/1-0048` exist even before the driver is loaded? The answer is that the device node in the kernel's internal bus list is created from the device tree entry, independently of whether any driver claims it. The device exists in the kernel's view of the world — it's simply unmanaged, with no driver attached yet — until a matching driver is registered.
